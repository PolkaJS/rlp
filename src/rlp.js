...
function decode_str(input: Buffer, r: bool = false) {
  let decoded = [];
  ...
  while (input[0]) {
    ...
    // one byte string or number that is less than 0x80
    if (input[0] < 128) {
      decoded.push(input.slice(0, 1).toString());
      input = bufferUnshift(input, 1);
      continue;
    }
    ...
    if (input[0] < 183) {
      ...
      decoded.push(input.slice(1, 1 + payloadLength).toString());
      input = bufferUnshift(input, 1 + payloadLength);
      continue;
    }
    ...
    if (input[0] < 192) {
      ...
      decoded.push(input.slice(1 + lengthSize, 1 + lengthSize + payloadLength).toString());
      input = bufferUnshift(input, 1 + lengthSize + payloadLength);
      continue;
    }
    ...
    if (input[0] < 247) {
      ...
      decoded.push( decode_str(input.slice(1, 1 + lengthSize), true) );
      input = bufferUnshift(input, 1 + lengthSize);
      continue;
    }
    ...
    decoded.push( decode_str(input.slice(1 + lengthSize), true) );
    input = bufferUnshift(input, 1 + lengthSize + payloadLength);
  }
...
module.exports = { encode, decode, decode_str };
